    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            ...
            <div class="panel-body">
                @can('create', 'App\Models\Contact')
                    <div class="space12">
                        <a class="btn btn-green add-row"
                           href="{{ route('contact.create') }}"
                        >
                            {{ __('admin.new_contact') }}
                    ...
                    <tbody>
                        @forelse ($contacts as $key => $contact)
                            <tr>
                                <td>{{ $contact->id ?? '' }}</td>
                                <td>{{ $contact->name ?? '' }}</td>
                                <td class="hidden-xs">{{ $contact->value ?? '' }}</td>
                                <td class="hidden-xs"><button class="btn btn-warning" disabled="disabled"><i class="{{ $contact->icon }}"></i></button></td>
                                <td class="hidden-xs"><span class="label label-success">{{ $contact->is_social ? 'YES' : 'NO' }}</span></td>
                                <td class="">
                                    <div class="visible-md visible-lg hidden-sm hidden-xs">
                                        @can('update', 'App\Models\Contact')
