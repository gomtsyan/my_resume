    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            ...
            <div class="panel-body">
                @can('create', 'App\Models\ArticleCategory')
                    <div class="space12">
                        <a class="btn btn-green add-row"
                           href="{{ route('category.create') }}"
                        >
                            {{ __('admin.new_article_category') }}
                ...
                <table class="table table-striped table-hover margin-bottom-10" id="sample-table-2">
                    ...
                    <tbody>
                        @forelse ($articleCategories as $key => $articleCategory)
                            <tr>
                                <td>{{ $articleCategory->id ?? '' }}</td>
                                <td>{{ $articleCategory->title ?? '' }}</td>
                                <td class="hidden-xs">{{ $articleCategory->slug ?? '' }}</td>
                                <td class="">
                                    <div class="visible-md visible-lg hidden-sm hidden-xs">
                                        @can('update', 'App\Models\ArticleCategory')
                                            <a href="{{ route('category.edit', ['category'=>$articleCategory->id]) }}"
                                        ...
                                        @endcan
                                        @can('delete', 'App\Models\ArticleCategory')
                                            <a href="{{ route('category.destroy', ['category'=>$articleCategory->id]) }}"
                                            ...
                                            <ul role="menu" class="dropdown-menu pull-right dropdown-dark">
                                                @can('update', 'App\Models\ArticleCategory')
                                                    <li>
                                                        <a role="menuitem"
                                                           tabindex="-1"
                                                           href="{{ route('category.edit', ['category'=>$articleCategory->id]) }}"
                                                        >
                                                ...
                                                @can('delete', 'App\Models\ArticleCategory')
                                                    <li>
                                                        <a role="menuitem"
                                                           class="delete"
                                                           tabindex="-1"
                                                           href="{{ route('category.destroy', ['category'=>$articleCategory->id]) }}"
                                                           data-type="button_in_table"
                                                           data-name="category"
                                                        >
