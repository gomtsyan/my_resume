    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            ...
            <div class="panel-body">
                @can('create', 'App\Models\LanguageSkill')
                    <div class="space12">
                        <a class="btn btn-green add-row"
                           href="{{ route('languages.create') }}"
                        >
                            {{ __('admin.new_language_skill') }}
                ...
                <table class="table table-striped table-hover margin-bottom-10" id="sample-table-2">
                    ...
                    <tbody>
                        @forelse ($languageSkills as $key => $languageSkill)
                            <tr>
                                <td>{{ $languageSkill->id ?? '' }}</td>
                                <td>{{ $languageSkill->name ?? '' }}</td>
                                <td class="hidden-xs">{{ $languageSkill->rating ?? '' }}</td>
                                <td class="hidden-xs">{{ $languageSkill->max_rating ?? '' }}</td>
                                <td class="hidden-xs"><span class="badge badge-warning">{{ $languageSkill->order ?? '' }}</span></td>
                                <td class="">
                                    <div class="visible-md visible-lg hidden-sm hidden-xs">
                                        @can('update', 'App\Models\LanguageSkill')
                                            <a href="{{ route('languages.edit', ['language'=>$languageSkill->id]) }}"
                                        ...
                                        @endcan
                                        @can('delete', 'App\Models\LanguageSkill')
                                            <a href="{{ route('languages.destroy', ['language'=>$languageSkill->id]) }}"
                                            ...
                                            <ul role="menu" class="dropdown-menu pull-right dropdown-dark">
                                                @can('update', 'App\Models\LanguageSkill')
                                                    <li>
                                                        <a role="menuitem"
                                                           tabindex="-1"
                                                           href="{{ route('languages.edit', ['language'=>$languageSkill->id]) }}"
                                                        >
                                                ...
                                                @can('delete', 'App\Models\LanguageSkill')
                                                    <li>
                                                        <a role="menuitem"
                                                           class="delete"
                                                           tabindex="-1"
                                                           href="{{ route('languages.destroy', ['language'=>$languageSkill->id]) }}"
                                                           data-type="button_in_table"
                                                           data-name="skill"
                                                        >
